...
// Update Temple by the id in the request
exports.update = (req, res) => {
  ...
  const templ_id = req.params.temple_id;
  ...
  Temple.findByIdAndUpdate({temple_id: templ_id}, updatedTemple, { new: true })
      .then((data) => {
      if (!data) {
        return res.status(404).send({
        message: `Cannot update Temple with id=${id}. Temple was not found!`,
      });
      }
      else res.send(data);
    })
